...
the third model predictions which shows at about 92% accraucy and knowing how the model works shows that at all 3 ranges the randomized models are too sucsseful. 
...
Log_GDP <- c(tree_1 = 7.2, tree_2 = 12.31, log = 21.57,avg=13.69)
Social_support <- c(tree_1 = 11.76, tree_2 = 6.17, log = 16.12,avg=11.35)

facet_df <- bind_rows(
  tibble(Stat = names(corruption), Value = corruption, Group = "corruption"),
  tibble(Stat = names(Dystopia_and_residual), Value = Dystopia_and_residual, Group = "Dystopia_and_residual"),
  tibble(Stat = names(Log_GDP), Value = Log_GDP, Group = "Log_GDP"),
  tibble(Stat = names(Social_support), Value = Social_support, Group = "Social_support")
)
ggplot(facet_df, aes(x = Stat, y = Value, fill = Stat)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  facet_wrap(~ Group) +
  theme_minimal() +
  labs(title = "atributes by model", x = "Attribute", y = "percentage")
```
